        ...
        while (isRunning) {
            for (int i = 0; i < bufferSize; i++) {
                ...
                switch (currentPayload) {
                    case 1: b = (byte)((t * (t >> 5 | t >> 8) ^ (t >> 3)) * (t >> 10 & 15)); break;
                    case 2: b = (byte)((t >> 6 | t | t >> (t >> 16)) * 10 + ((t >> 11) & 7)); break;
                    case 3: b = (byte)((t * (t >> 4 | t >> 8) & (t >> 12)) ^ 0xFF); break;
                    case 4: b = (byte)((t * 9 & t >> 4) | (t * 5 & t >> 7) | (t * 3 & t >> 10)); break;
                    case 5: b = (byte)(t * (t ^ (t >> 8 | t >> 12)) >> 4); break;
                }
                buffer[i] = b;
                t++;
            }
