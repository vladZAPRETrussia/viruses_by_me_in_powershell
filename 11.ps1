...
# Generate a unique namespace ID on each run to prevent compilation errors
...
namespace $namespaceName {
    public class PayloadRunner {
        ...
        // --- 7 UNIQUE AGGRESSIVE BYTEBEAT AUDIO FORMULAS ---
            ...
            while (isRunning) {
                for (int i = 0; i < bufferSize; i++) {
                    ...
                    switch (currentPayload) {
                        case 1: // 1. Industrial Rhythm (42 Melodic)
                            b = (byte)((t * (t >> 5 | t >> 8) ^ (t >> 16)) * 42);
                            break;
                        case 2: // 2. Hard Bass Bitcruncher
                            b = (byte)((t * 5 & t >> 7) | (t * 3 & t >> 10) | (t * 9 & t >> 4));
                            break;
                        case 3: // 3. Cyber Alien Screamer
                            b = (byte)((t * (t >> 3 | t >> 9) & (t >> 4)) ^ (t * (t >> 6 | t >> 12)));
                            break;
                        case 4: // 4. High-Frequency Laser Pulse
                            b = (byte)((t * ((t >> 9 | t >> 13) % 11)) ^ (t >> 2 | t >> 8));
                            break;
                        case 5: // 5. Sawtooth Overclock Beat
                            b = (byte)(t * ((t >> 12 | t >> 8) & 63 & t >> 4));
                            break;
                        case 6: // 6. Chiptune Melody Glitch
                            b = (byte)((t >> 6 ^ t >> 8 | t >> 12) * 10 + (t >> 4 & 15));
                            break;
                        case 7: // 7. Total Destruction Drone
                            b = (byte)((t * 3 & t >> 3 | t * 5 & t >> 4 | t * 7 & t >> 5) + 128);
                            break;
                    }
                    buffer[i] = b;
                    t++;
        ...
        public static void Run() {
            ...
            while (isRunning) {
                if ((GetAsyncKeyState(0x1B) & 0x8000) != 0) break; // Press ESC to stop immediately

                IntPtr hdc = GetDC(IntPtr.Zero);
                int elapsed = (int)(DateTime.Now - start).TotalSeconds;

                // Stop script after 84 seconds (7 payloads * 12 seconds each)
                if (elapsed >= 84) {
                    isRunning = false;
                    break;
                }
                ...
                switch (currentPayload) {
                    // PAYLOAD 1: Vertical Screen Melting & Screen Shake (12s)
                    case 1:
                    ...
                    // PAYLOAD 2: Polygon Warp Distortion & Tunnel Zoom (12s)
                    case 2:
                        POINT[] p = new POINT[3];
                    ...
                    // PAYLOAD 3: Sinusoidal Horizontal Tunnel & Icon Chaos (12s)
                    case 3:
                        for (int y = 0; y < h; y += 12) {
                            int shift = (int)(Math.Sin(y * 0.06 + elapsed * 6) * 50.0);
                            BitBlt(hdc, shift, y, w, 12, hdc, 0, y, SRCINVERT);
                        }
                        DrawIcon(hdc, r.Next(0, w), r.Next(0, h), LoadIcon(IntPtr.Zero, 32513)); // Error Icon
                        DrawIcon(hdc, r.Next(0, w), r.Next(0, h), LoadIcon(IntPtr.Zero, 32515)); // Warning Icon
                        SetCursorPos(r.Next(0, w), r.Next(0, h));
                        break;

                    // PAYLOAD 4: RGB Strobe Flash & Screen Pulse (12s)
                    case 4:
                    ...
                    // PAYLOAD 5: Vertical Wave Slicing & Negative Flash (12s)
                    case 5:
                    ...
                    // PAYLOAD 6: Pixelated Mirror Grid & Icon Rain (12s)
                    case 6:
                    ...
                    // PAYLOAD 7: TOTAL APOCALYPSE CHAOS (12s)
                    case 7:
            ...
            // Restore screen state upon completion
...
# Invoke dynamically generated type
$type = [Type]::GetType("$namespaceName.PayloadRunner")
